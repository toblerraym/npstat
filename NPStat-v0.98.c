  ...
  for(rd=2*m_bar+1;rd<=max_cov; rd++){
   ...
   for(k=1;k<(n01-1);k++){
     ...
     for(covt=1;covt<=m_bar;covt++){
	      ...
	      vec_p[(k-1)+(n01-1)*(rd-1)]+=-( 2*covt*(rd-covt)/(rd*(rd-1))*gsl_ran_binomial_pdf(covt,	(double)k/(double)n01,rd)+gsl_ran_binomial_pdf(rd-covt,(double)k/(double)n01,rd)); 
	      vec_h[(k-1)+(n01-1)*(rd-1)]+=-( 2*covt*covt/(rd*(rd-1))*gsl_ran_binomial_pdf(covt,	(double)k/(double)n01,rd)+gsl_ran_binomial_pdf(rd-covt,(double)k/(double)n01,rd)); 
     };       
...
/*      if (pos_base2<pos)
      ...
      if ((pos_base1==pos)&&(rd1>=min_cov)&&(rd1<=max_cov))
	{
	  ...
	  if (rd1>=max(min_cov,2*m_bar+2))
		...
		if(strand=='+'){
	      ...
	      extract_stats(&test1, &comb1, n01, n_ref1, n_alt_allele1, rd1, n_alt_1, ref_base1, alt_base1, out_base, m_bar);
	      if(out_base!=0){
      ...
      if ((pos==(n_window*window_size))||(ct1==EOF))
	{
	  ...
	  if(dsyn*pnon==0){
	    ...
	    fprintf(output_stat1, "\t%f", 1-(double)(psyn*dnon)/(double)(dsyn*pnon));
	  };
	  } else { 
